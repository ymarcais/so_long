...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/22 13:36:19 by ymarcais          #+#    #+#             */
/*   Updated: 2023/02/22 13:51:11 by ymarcais         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_check_col(char *map_line, char wall, t_data  *data)
{
    ...
    while (map_line[i])
        i++;
    if (map_line[i] != wall || map_line[i - 1] != wall)
    {
        ft_error("Error\nMap column not closed\n");
        return (0);
    }
    data->width = 1;
    return (1);
...
int ft_check_other(char *map_line, t_data *data)
{
    int i;
    ...
    while (map_line[i])
    {
        ...
        if (map_line[i] != data->wall && map_line[i] != content->player
...
void    ft_check_content(d_data *data)
{
    int i;
    int y;
